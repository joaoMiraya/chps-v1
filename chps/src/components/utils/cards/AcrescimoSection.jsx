        ...
        <>
            ...
            <div className="flex flex-wrap gap-2 justify-center">
                {acrescimos.map((acrescimo) => {
                    return (
                        <span key={acrescimo.id}
                            onClick={() => handleSelectAcrescimo(acrescimo.id)}
                            className={`px-2 py-[2px] rounded-full  ${selectedAcrescimos.includes(acrescimo.id) ? "border-[1px] border-solid border-green-400" : "border-[1px] border-solid border-gray-400"
                                }`}>
                            + {acrescimo.nome}
                        </span>
