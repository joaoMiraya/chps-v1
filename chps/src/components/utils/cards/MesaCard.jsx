...
function MesaCard({ mesaNumero }) {
    ...
    return (
        ...
        <Link to={`/mesas/${mesaNumero}`} className="flex rounded-xl w-[10rem]  relative items-center shadow-md cursor-pointer hover:scale-105 transition-all duration-300  gap-2 ">
            <span className={` ${mesa ? 'mesaActive' : 'mesaDefault'} shadow-sm flex justify-center items-center w-[4rem] h-[4rem] relative top-1/2`}>
                <img className='absolute' src={icon} alt="" />
            </span>
            <span className={`${mesa ? 'bg-green-600' : ''} absolute drop-shadow-md right-2 top-2 w-[.7rem] h-[.7rem]  bg-gray-300 rounded-full`}></span>
